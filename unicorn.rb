...
dep 'unicorn.monitored' do
  met? { "/etc/monit/conf.d/unicorn_master.monitrc".p.exists? && "/etc/monit/conf.d/unicorn_worker.monitrc".p.exists? }
...
dep 'unicorn.startable', :user do
  ...
  met? { "/etc/init/unicorn.conf".p.exists? && "/etc/init.d/unicorn".p.exists? }
  meet do
